...
function addConfirm(message, onconfirm) {
  var m = document.createElement('div');
  m.className = 'confirm';
  m.innerHTML = message + '<a href="#" class="btn error yes">' + _('Yes') +
    '</a> <a href="#" class="btn cancel">' + _('Cancel') + '</a>';
...
document.addEventListener('close', function (e) {
  ...
  if (t && t.matches('.tooltip')) {
    each('.select.multiple.open', function (i) {
      trg(i, 'change');
    }, t)
    setTimeout(function () {
      t.remove();
...
var Editor = function (dom, onchange) {
  ...
  evt(dom, 'click', function (e) {
    ...
    if (t.matches('.plus-one')) {
      ...
      if (self.chunks.length > cid + 1) {
        t.dataset.next = cid + 1;  // TODO next is string?
      } else {
        t.remove();
      }
    }
  ...
  window.addEventListener('beforeunload', function (e) {
    if (self.ischanged()) {
      e.preventDefault();
      e.returnValue = _('There are unsaved changes! Are you sure?');  // TODO deprecated
...
Editor.prototype.render = function (cids) {
  ...
  if (self.chunks.length > cids[cids.length - 1] + 1) {
    ...
    a.dataset.next = parseInt(cids[cids.length - 1]) + 1;  // TODO incompatible types
...
Editor.prototype.renderPaginator = function (cid) {
  ...
  for (var p = Math.max(1, cur - 3); p <= s; ++p) {
    html += '<li class="' + (p == cur ? 'active' : '') + '"><a href="#" class="btn" data-render="' + (p - 1) + '">'
      + p + '</a></li>';
  }
  if (max > cur + 3) {
    html += '<li class="disabled"><a>...</a></li>';
...
History.prototype.onchange = function () {
  ...
  this.timer = setTimeout(function () {
    self.backup()
  }, Math.max(200, (this.bu_stamp || 0) + History.BACKUP_INTERVAL - new Date().getTime()));
  if (this.onchange_cb) this.onchange_cb(this);
}
...
function open(id, onsuccess, template) {
  ...
  if (id !== undefined) {
    ...
    promise = templatePromise
      .then(template => chooseFile(template.extension)  // TODO unresolved variable extension
